...
import {
    ...
    Search, X, Eye, Printer, Check, ChevronRight, Plus, Menu,
} from "lucide-react";
...
// =============================================================
const ALL_CERTS = [
    { name: "Barangay Clearance",                      hasFee: true,  desc: "For general official purposes. Certifies the holder is a resident in good standing." },
    { name: "Certificate of Residency",                hasFee: false, desc: "Certifies that the individual is a bona fide resident of Barangay East Tapinac." },
    { name: "Certificate of Indigency",                hasFee: false, desc: "Certifies that the individual belongs to an indigent family in the barangay." },
    { name: "Business Permit",                         hasFee: true,  desc: "Authorizes the holder to operate a business within Barangay East Tapinac." },
    { name: "Good Moral Certificate",                  hasFee: false, desc: "Certifies that the individual is of good moral character in the community." },
    { name: "Certificate of Live Birth (Endorsement)", hasFee: false, desc: "Endorsement for late registration of live birth at the civil registry." },
    { name: "Certificate of Cohabitation",             hasFee: false, desc: "Certifies that two individuals are cohabiting as a couple within the barangay." },
    { name: "Certificate of No Business",              hasFee: false, desc: "Certifies that the individual has no registered business in the barangay." },
    { name: "Certificate of Guardianship",             hasFee: false, desc: "Certifies guardianship relationship for use in official transactions." },
    { name: "Certificate of Late Registration",        hasFee: false, desc: "For late registration of vital civil events with the local civil registry." },
    { name: "Barangay Business Clearance (Renewal)",   hasFee: true,  desc: "Annual renewal of barangay business clearance for existing permit holders." },
    { name: "Certificate of Ownership",                hasFee: false, desc: "Certifies ownership of property or assets within the jurisdiction of the barangay." },
];
...
const MOCK_LOG = [
    { id: "#WI-034", name: "Felicidad Ramos",    type: "Barangay Clearance",  purpose: "Employment",        issuedBy: "Staff Reyes", time: "10:14 AM" },
    { id: "#WI-033", name: "Ernesto Villanueva", type: "Cert. of Indigency",  purpose: "Medical Assistance", issuedBy: "Staff Cruz",  time: "9:02 AM"  },
    { id: "#WI-032", name: "Natividad Santos",   type: "Cert. of Residency",  purpose: "Senior Citizen ID",  issuedBy: "Staff Reyes", time: "8:30 AM"  },
];
    ...
    return (
        <aside style={{ ...sd.sidebar, width: collapsed ? 60 : 240 }}>
            <div style={{ ...sd.brand, justifyContent: collapsed ? "center" : "flex-start", padding: collapsed ? "18px 0" : "20px 20px 16px" }}>
                ...
                {!collapsed && <div><div style={sd.brandName}>CertiFast</div><div style={sd.brandSub}>East Tapinac</div></div>}
            </div>
            <div style={sd.goldBar} />
            {!collapsed && <div style={sd.sectionLabel}>Main Menu</div>}
            {navItems.map(({ key, label, Icon, badge }) => collapsed ? (
                <button key={key} className={`wi-nav-item-icon${activePage === key ? " active" : ""}`} onClick={() => onNavigate(key)} title={label}>
                    <Icon size={18} opacity={0.7} />
                </button>
            ) : (
                <button key={key} className={`wi-nav-item${activePage === key ? " active" : ""}`} onClick={() => onNavigate(key)}>
            ...
            {isSuperAdmin && (
                <div style={sd.superAdminSection}>
                    ...
                    {saItems.map(({ key, label, Icon }) => collapsed ? (
                        <button key={key} className={`wi-nav-item-icon${activePage === key ? " active" : ""}`} onClick={() => onNavigate(key)} title={label}>
                            <Icon size={18} opacity={0.7} />
                        </button>
                    ) : (
            ...
            <div style={{ ...sd.userRow, justifyContent: collapsed ? "center" : "flex-start", padding: collapsed ? "14px 0" : "14px 20px" }}>
                ...
                {!collapsed && (
                    <>
                        ...
                        <button className="wi-logout-btn" onClick={onLogout}><LogOut size={14} /></button>
                    </>
                )}
            </div>
        </aside>
            ...
            <aside style={{ ...sd.sidebar, width: 260, position: "relative", animation: "sidebarSlideIn 0.22s ease both", zIndex: 1 }}>
                <div style={{ ...sd.brand, justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={sd.brandSeal}><span style={{ fontSize: 14, color: "rgba(201,162,39,0.6)" }}>⚜</span></div>
                        <div><div style={sd.brandName}>CertiFast</div><div style={sd.brandSub}>East Tapinac</div></div>
                    </div>
                    <button style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.5)", padding: 4 }} onClick={onClose}><X size={18} /></button>
                </div>
                <div style={sd.goldBar} />
                <div style={sd.sectionLabel}>Main Menu</div>
                {navItems.map(({ key, label, Icon, badge }) => (
                    <button key={key} className={`wi-nav-item${activePage === key ? " active" : ""}`} onClick={() => { onNavigate(key); onClose(); }}>
                        <Icon size={15} opacity={0.7} />{label}
                        {badge && <span style={sd.navBadge}>{badge}</span>}
                    </button>
                ))}
...
function CertPickerModal({ onClose, onPick }) {
    ...
    const filtered = ALL_CERTS.filter(c => !query || c.name.toLowerCase().includes(query.toLowerCase()));
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
            ...
            <div style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 560, maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.25)" }}>
                {/* Header */}
                <div style={{ background: "linear-gradient(135deg,#0e2554,#163066)", padding: "18px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
                    <div>
                        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 15, fontWeight: 700, color: "#fff" }}>All Certificate &amp; Permit Types</div>
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)", marginTop: 2 }}>Click any type to select · Manage cert types in System Settings</div>
                    </div>
                    <button onClick={onClose} style={{ background: "rgba(255,255,255,.1)", border: "none", borderRadius: 4, color: "#fff", width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <X size={13} strokeWidth={2.5} />
                    </button>
                </div>
                {/* Search */}
                <div style={{ padding: "12px 18px", borderBottom: "1px solid #e4dfd4", flexShrink: 0, display: "flex", alignItems: "center", gap: 8 }}>
                    <Search size={14} color="#9090aa" strokeWidth={2} />
                    <input ref={inputRef} className="wi-modal-input" type="text" placeholder="Search certificate or permit type…"
                        value={query} onChange={(e) => setQuery(e.target.value)} />
                </div>
                {/* List */}
                <div style={{ overflowY: "auto", flex: 1 }}>
                    {filtered.length === 0 ? (
                        <div style={{ padding: 32, textAlign: "center", color: "#9090aa", fontSize: 12.5 }}>
                            No results for "<strong>{query}</strong>"
                        </div>
                    ) : filtered.map((cert) => (
                        <div key={cert.name} className="wi-modal-item" onClick={() => { onPick(cert); onClose(); }}>
                            <div style={{ width: 38, height: 38, borderRadius: 8, background: "#f3eeff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                <FileText size={18} color="#6a3db8" strokeWidth={1.5} />
                            </div>
                            <div style={{ flex: 1 }}>
                                <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1a2e" }}>{cert.name}</div>
                                <div style={{ fontSize: 11, color: "#9090aa", marginTop: 2 }}>{cert.desc}</div>
                            </div>
                            <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 10, flexShrink: 0, background: cert.hasFee ? "#fff3e0" : "#e8f5ee", color: cert.hasFee ? "#b86800" : "#1a7a4a" }}>
...
function PrintPreviewModal({ cert, formData, docId, onClose, onConfirm }) {
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
            onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 640, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
                {/* Header */}
                <div style={{ background: "linear-gradient(135deg,#0e2554,#163066)", padding: "16px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
                    <div>
                        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 15, fontWeight: 700, color: "#fff" }}>Print Preview</div>
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,.5)", marginTop: 2 }}>Review before printing — click Confirm &amp; Print to log and issue</div>
                    </div>
                    <button onClick={onClose} style={{ background: "rgba(255,255,255,.1)", border: "none", borderRadius: 4, color: "#fff", width: 30, height: 30, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <X size={13} strokeWidth={2.5} />
                    </button>
                ...
                {/* Paper preview */}
                <div style={{ flex: 1, overflowY: "auto", padding: 28, background: "#e8e4dc", display: "flex", justifyContent: "center" }}>
                    <div className="wi-cert-paper">
                        {/* Header */}
                        <div style={{ textAlign: "center", marginBottom: 18 }}>
                            <div style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: "#9090aa" }}>Republic of the Philippines</div>
                            <div style={{ fontSize: 11, fontWeight: 600, color: "#0e2554", marginTop: 2 }}>City Government of Olongapo · Zambales</div>
                            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 17, fontWeight: 700, color: "#0e2554", marginTop: 2 }}>Barangay East Tapinac</div>
                            <div style={{ fontSize: 10, color: "#9090aa", marginTop: 3 }}>54 – 14th Street corner Gallagher Street, Olongapo City</div>
                        </div>
                        <hr style={{ border: "none", borderTop: "2px solid #c9a227", margin: "14px 0 10px" }} />
                        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, fontWeight: 700, color: "#0e2554", textAlign: "center", letterSpacing: 1, marginBottom: 3 }}>
                            {cert.name.toUpperCase()}
                        </div>
                        <div style={{ fontSize: 10, textAlign: "center", letterSpacing: 2, textTransform: "uppercase", color: "#9090aa", marginBottom: 18 }}>
                            Office of the Punong Barangay
                        </div>
                        <div style={{ fontSize: 12, lineHeight: 1.9, color: "#1a1a2e", textAlign: "justify" }}
                        ...
                        <div style={{ marginTop: 32, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                            <div style={{ fontSize: 11, color: "#4a4a6a" }}>
                                Issued at Barangay East Tapinac<br/>
                                <strong style={{ display: "block", fontSize: 12, color: "#1a1a2e", marginTop: 2 }}>{dateStr}</strong>
                            </div>
                            <div style={{ textAlign: "center" }}>
                                <div style={{ width: 180, borderTop: "1.5px solid #0e2554", paddingTop: 4, marginTop: 40 }}>
                                    <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 12, fontWeight: 700, color: "#0e2554" }}>HON. DANTE L. HONDO</div>
                                    <div style={{ fontSize: 10, color: "#9090aa", marginTop: 1 }}>Punong Barangay</div>
                                </div>
                            </div>
                        </div>
                        <div style={{ position: "absolute", bottom: 12, right: 16, fontFamily: "'Courier New',monospace", fontSize: 9, color: "#bbb" }}>{docId}</div>
                    </div>
                </div>
                {/* Footer */}
                <div style={{ padding: "14px 22px", borderTop: "1px solid #e4dfd4", display: "flex", gap: 10, justifyContent: "flex-end", background: "#fff", flexShrink: 0 }}>
                    <button onClick={onClose} style={{ padding: "9px 20px", background: "#fff", border: "1.5px solid #e4dfd4", borderRadius: 4, fontFamily: "'Source Serif 4',serif", fontSize: 12.5, color: "#4a4a6a", cursor: "pointer" }}>
                        ← Back to Edit
                    </button>
...
// Main Component
// =============================================================
export default function WalkInIssuance({ admin, onLogout, onNavigate: navProp }) {
    ...
    const [formData, setFormData] = useState({ residentName: "", address: "", purpose: "" });
    ...
    const handleConfirmPrint = () => {
        ...
        showToast(`${newEntry.id} — ${shortName(selectedCert.name)} issued and logged.`);
        // Reset
    ...
    const filteredLog = log.filter(entry =>
        !logQuery || [entry.id, entry.name, entry.type, entry.purpose].join(" ").toLowerCase().includes(logQuery.toLowerCase())
    );
    ...
    return (
        <div className="wi-root">
            ...
            <div style={{ marginLeft: sidebarWidth, flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
                ...
                {/* Topbar */}
                <div style={{ ...tb.topbar, padding: isMobile ? "0 16px" : "0 32px" }}>
                    {isMobile && (
                    ...
                    <div style={tb.topbarTitle}>
                        Walk-in Issuance
                        {!isMobile && <span style={tb.topbarSub}>Issue certificates directly at the counter</span>}
                    </div>
                    {!isMobile && (
                ...
                <div style={{ padding: isMobile ? "16px 16px 24px" : "28px 32px", flex: 1 }}>
                    ...
                    <div style={p.panel}>
                        {/* Panel header */}
                        <div style={{ ...p.panelHeader, background: "linear-gradient(135deg,#f3eeff,#ede8ff)", borderBottomColor: "#d4c8f0" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <div style={{ width: 32, height: 32, background: "#6a3db8", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                    <FilePlus size={16} color="white" strokeWidth={2} />
                                </div>
                                <div>
                                    <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 14, fontWeight: 700, color: "#4a1fa8" }}>Walk-in / Manual Issuance</div>
                                    <div style={{ fontSize: 11, color: "#7a5ab8", marginTop: 2 }}>Select a certificate type, fill in details, preview, then print &amp; log.</div>
                                </div>
                            </div>
                        </div>

                        {/* Step 1 label */}
                        <div style={{ padding: "12px 22px 8px", background: "#faf8ff", borderBottom: "1px solid #ede8ff" }}>
                            <span style={{ fontSize: 10, color: "#9090aa", letterSpacing: "1.2px", textTransform: "uppercase", fontWeight: 600 }}>
                                Step 1 — Select Certificate / Permit Type
                            </span>
                        </div>

                        {/* Quick cert cards */}
                        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", padding: "18px 22px", borderBottom: "1px solid #e4dfd4" }}>
                            {QUICK_CERTS.map((cert) => (
                                ...
                                >
                                    {cert.hasFee && (
                                        <span style={{ position: "absolute", top: 7, right: 7, fontSize: 8.5, fontWeight: 700, color: "#b86800", background: "#fff3e0", borderRadius: 8, padding: "1px 5px" }}>₱ Fee</span>
                                    )}
                                    <FileText size={20} color={selectedCert?.name === cert.name ? "#6a3db8" : "#0e2554"} strokeWidth={1.5} />
                                    <span style={{ fontSize: 11, fontWeight: 600, color: selectedCert?.name === cert.name ? "#4a1fa8" : "#1a1a2e", textAlign: "center", lineHeight: 1.3 }}>
                                        {cert.name.replace("Certificate of ", "Cert. of ").replace("Barangay ", "Brgy. ")}
                                    </span>
                                </button>
                            ))}
                            {/* View All */}
                            <button className="wi-cert-btn view-more" onClick={() => setShowCertModal(true)} style={{ minWidth: isMobile ? 80 : 100 }}>
                                <Plus size={20} color="#6a3db8" strokeWidth={1.5} />
                                <span style={{ fontSize: 11, fontWeight: 600, color: "#6a3db8", textAlign: "center", lineHeight: 1.3 }}>View All<br/>Cert Types</span>
                                <span style={{ fontSize: 8.5, color: "#9090aa" }}>Select only</span>
                            </button>
                        </div>

                        {/* Step 2 form — shown when cert is selected */}
                        {selectedCert && (
                            <div style={{ padding: "20px 22px", background: "#faf8ff", borderTop: "1px solid #ede8ff", animation: "wi-fadein .2s ease both" }}>
                                <div style={{ fontSize: 10, color: "#9090aa", letterSpacing: "1.2px", textTransform: "uppercase", fontWeight: 600, marginBottom: 12 }}>
                                    Step 2 — Fill in Resident Details
                                </div>

                                {/* Selected cert bar */}
                                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#f3eeff", border: "1.5px solid #c8b8e8", borderRadius: 5, marginBottom: 16 }}>
                                    <Check size={14} color="#6a3db8" strokeWidth={2} />
                                    <span style={{ fontSize: 12.5, fontWeight: 700, color: "#4a1fa8" }}>{selectedCert.name}</span>
                                    <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 10, background: selectedCert.hasFee ? "#fff3e0" : "#e8f5ee", color: selectedCert.hasFee ? "#b86800" : "#1a7a4a" }}>
                                        {selectedCert.hasFee ? "₱ Fee required" : "✓ No fee"}
                                    </span>
                                </div>

                                {/* Error */}
                                {formError && (
                                    <div style={{ background: "rgba(176,32,32,.07)", border: "1px solid rgba(176,32,32,.25)", borderRadius: 4, padding: "8px 12px", marginBottom: 12, fontSize: 12, color: "#b02020" }}>
                                        {formError}
                                    </div>
                                )}
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr auto", gap: 12, alignItems: "flex-end" }}>
                                    <div>
                                        ...
                                        <input className="wi-input" name="residentName" type="text" placeholder="e.g. Juan dela Cruz"
                                            value={formData.residentName} onChange={handleFormChange} />
                                    </div>
                                    <div>
                                        <div style={f.label}>Address <span style={{ color: "#b02020" }}>*</span></div>
                                        <input className="wi-input" name="address" type="text" placeholder="Street, East Tapinac"
                                            value={formData.address} onChange={handleFormChange} />
                                    </div>
                                    <div>
                                        <div style={f.label}>Purpose <span style={{ color: "#b02020" }}>*</span></div>
                                        <input className="wi-input" name="purpose" type="text" placeholder="e.g. Employment"
                                            value={formData.purpose} onChange={handleFormChange} />
                                    </div>
                    ...
                    <div style={p.panel}>
                        ...
                        <div style={p.panelHeader}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 14, fontWeight: 700, color: "#0e2554" }}>Today's Walk-in Issuances</div>
                                <span style={{ fontSize: 10, fontWeight: 700, background: "#f3eeff", color: "#6a3db8", padding: "2px 9px", borderRadius: 10 }}>
                                    {log.length} issued
                                </span>
                            </div>
                            <span style={{ fontSize: 11, color: "#9090aa" }}>{formatDateShort()}</span>
                        </div>

                        {/* Log search */}
                        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 22px", borderBottom: "1px solid #e4dfd4", background: "#f8f6f1" }}>
                            <Search size={14} color="#9090aa" strokeWidth={2} />
                            <input className="wi-log-input" type="text" placeholder="Search by name, certificate type, or Doc. ID…"
                                value={logQuery} onChange={(e) => setLogQuery(e.target.value)} />
                        </div>
                        ...
                        {!isMobile ? (
                            <div style={{ overflowX: "auto" }}>
                                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                                    <thead>
                                        <tr>
                                            {["Doc. ID", "Resident Name", "Certificate Type", "Purpose", "Issued By", "Time", "Action"].map(h => (
                                    ...
                                    <tbody>
                                        {filteredLog.length === 0 ? (
                                            <tr><td colSpan={7} style={{ textAlign: "center", color: "#9090aa", fontSize: 12, padding: 28 }}>No records match your search.</td></tr>
                                        ) : filteredLog.map((entry, i) => (
                                            <tr key={i} className="wi-tr">
                                                <td style={p.td}><span style={{ fontFamily: "'Courier New',monospace", fontSize: 11, color: "#9090aa" }}>{entry.id}</span></td>
                                                <td style={p.td}><div style={{ fontWeight: 600 }}>{entry.name}</div></td>
                                                <td style={p.td}><span style={{ fontSize: 11.5, color: "#4a4a6a" }}>{entry.type}</span></td>
                                                <td style={p.td}><span style={{ fontSize: 11.5, color: "#4a4a6a" }}>{entry.purpose}</span></td>
                                                <td style={p.td}><span style={{ fontSize: 11.5, color: "#4a4a6a" }}>{entry.issuedBy}</span></td>
                                                <td style={p.td}><span style={{ fontSize: 11, color: "#9090aa" }}>{entry.time}</span></td>
                                                <td style={p.td}>
                                                    {/* TODO: GET /api/walkin/:id/reprint */}
                                                    <button className="wi-action-btn">Reprint</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                            ...
                            <div>
                                {filteredLog.length === 0 ? (
                                    <div style={{ textAlign: "center", color: "#9090aa", fontSize: 12, padding: 28 }}>No records match your search.</div>
                                ) : filteredLog.map((entry, i) => (
                                    <div key={i} style={{ padding: "14px 16px", borderBottom: "1px solid #f0ece4", display: "flex", flexDirection: "column", gap: 6 }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                            <div>
                                                <div style={{ fontWeight: 600, fontSize: 13 }}>{entry.name}</div>
                                                <div style={{ fontSize: 11.5, color: "#4a4a6a", marginTop: 2 }}>{entry.type} · {entry.purpose}</div>
                                            </div>
                                            <button className="wi-action-btn">Reprint</button>
                                        </div>
                                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                                            <span style={{ fontFamily: "'Courier New',monospace", fontSize: 11, color: "#9090aa" }}>{entry.id}</span>
                                            <span style={{ fontSize: 11, color: "#9090aa" }}>{entry.issuedBy} · {entry.time}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
...
const sd = {
    ...
    brandSeal:  { width: 38, height: 38, borderRadius: "50%", border: "1.5px solid rgba(201,162,39,0.5)", background: "rgba(201,162,39,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
...
const tb = {
    ...
    topbarTitle: { fontFamily: "'Playfair Display',serif", fontSize: 18, fontWeight: 700, color: "#0e2554", flex: 1 },
    topbarSub:   { fontSize: 12, fontFamily: "'Source Serif 4',serif", color: "#9090aa", fontWeight: 400, marginLeft: 10 },
...
const p = {
    panel:      { background: "#fff", border: "1px solid #e4dfd4", borderRadius: 6, overflow: "hidden", marginBottom: 20 },
    panelHeader:{ padding: "16px 22px", borderBottom: "1px solid #e4dfd4", display: "flex", alignItems: "center", justifyContent: "space-between" },
    th:         { fontSize: 9.5, fontWeight: 600, color: "#9090aa", letterSpacing: "1.2px", textTransform: "uppercase", padding: "10px 22px", textAlign: "left", background: "#f8f6f1", borderBottom: "1px solid #e4dfd4", whiteSpace: "nowrap" },
    td:         { padding: "12px 22px", fontSize: 12.5, color: "#1a1a2e", borderBottom: "1px solid #f0ece4", verticalAlign: "middle", transition: "background 0.1s" },
};

const f = {
    label: { fontSize: 10, fontWeight: 600, color: "#4a4a6a", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 6 },
};
