...
import {
    Eye, EyeOff, Mail, Lock, User, MapPin,
    Phone, Hash, ChevronLeft, ChevronRight,
    Check, Upload, X, AlertCircle, CalendarClock, CalendarDays,
} from "lucide-react";
...
const ID_TYPES = [
    "Barangay ID","Voter's ID","PhilSys National ID",
    "Driver's License","Passport","School ID","Postal ID","Other valid ID",
];
    ...
    return (
        <div style={{ marginBottom: mb ?? 14, ...style }}>
            <label className="reg-label">
                {label}{required && <span style={{ color:"#b02020", marginLeft:2 }}>*</span>}
            </label>
            {children}
        </div>
    );
...
function IconWrap({ icon: Icon, children }) {
    return (
        <div style={{ position:"relative" }}>
            <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", opacity:0.35, pointerEvents:"none", display:"flex", zIndex:1 }}>
                ...
                return (
                    <div key={n} style={{ display:"flex", alignItems:"center", flex: i < steps.length - 1 ? "1" : "0" }}>
                        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:5, flexShrink:0 }}>
                            <div className={`reg-step-dot ${isDone?"done":isActive?"active":"idle"}`}>
                                {isDone ? <Check size={14} strokeWidth={2.5}/> : n}
                            </div>
                            <span style={{ fontSize:10, color:isActive?"#0e2554":isDone?"#1a7a4a":"#9090aa", fontWeight:isActive||isDone?700:400, whiteSpace:"nowrap" }}>
                                {label}
                            </span>
                        </div>
                        {i < steps.length - 1 && <div className={`reg-step-line${isDone?" done":""}`}/>}
                    </div>
                );
            })}
        </div>
...
export default function ResidentRegister({ onSuccess }) {
    ...
    useEffect(() => {
        // TODO (Backend Dev): GET /api/address-options
        //   → { puroks: [{purok_id, name},...], streets: [{street_id, name},...] }
        async function loadAddressOptions() {
            try {
                const res = await fetch("/api/address-options");
                if (!res.ok) return;
                const data = await res.json();
                if (Array.isArray(data.puroks)  && data.puroks.length)  setPuroks(data.puroks);
                if (Array.isArray(data.streets) && data.streets.length) setStreets(data.streets);
            } catch {
    ...
    const [form, setForm] = useState({
        ...
        house_no:"", purok_id:"", street_id:"", street_other:"", date_of_birth:"", civil_status:"", nationality:"Filipino",
    });
    ...
    const selectedStreet  = streets.find(s => String(s.street_id) === String(form.street_id));
    const resolvedStreet  = form.street_id === "other"
        ? form.street_other.trim()
        : selectedStreet?.name ?? "";
    const selectedPurok   = puroks.find(p => String(p.purok_id) === String(form.purok_id));
    ...
    const handleFilePick = (e) => {
        ...
        setError(""); setIdFile(file); setIdPreview(URL.createObjectURL(file));
    };
    const removeFile = () => {
    ...
    const validateStep = () => {
        if (step === 1) {
            ...
            if (!form.house_no.trim())   return "House / Unit number is required.";
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const address_street = `${resolvedStreet}, Barangay East Tapinac, Olongapo City`.trim();
            ...
            if (form.civil_status)  payload.append("civil_status",  form.civil_status);
    ...
    if (success) {
        return (
            <div className="reg-root" style={{ alignItems:"center", padding:"28px 16px" }}>
                <div className="reg-card reg-pop" style={{ maxWidth:600 }}>
                    <div style={{ height:4, background:"linear-gradient(90deg,#c9a227,#f0d060,#c9a227)" }} />
                    <div style={{ padding:"48px 36px", textAlign:"center" }}>
                        <div style={{ width:68, height:68, borderRadius:"50%", background:"#e8f5ee", border:"2px solid #1a7a4a", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 20px" }}>
                            <Check size={30} color="#1a7a4a" strokeWidth={2.5}/>
                        </div>
                        <p style={{ fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:700, color:"#0e2554", margin:"0 0 10px" }}>
                            Account Submitted!
                        </p>
                        <p style={{ fontSize:13.5, color:"#4a4a6a", lineHeight:1.7, margin:"0 0 20px" }}>
                            Your registration has been received. Barangay staff will review your submitted ID photo.
                        </p>
                        <div style={{ background:"#fff7e6", border:"1px solid #f5d78e", borderRadius:7, padding:"14px 18px", marginBottom:28, textAlign:"left", display:"flex", gap:12 }}>
                            <AlertCircle size={17} color="#b86800" style={{ flexShrink:0, marginTop:2 }}/>
                            <div style={{ fontSize:13, color:"#7a4800", lineHeight:1.7 }}>
                                <strong>Please come back in 1–3 business days</strong> to log in once your account has been verified and approved by the barangay.
                            </div>
                        </div>
                        <button className="reg-btn-primary" style={{ width:"100%", justifyContent:"center" }}
        ...
        <div className="reg-fadein">
            ...
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap:12, marginBottom:14 }}>
                <FieldGroup label="First Name" required mb={0}>
                    <IconWrap icon={User}>
                        <input className="reg-input" type="text" value={form.first_name}
                ...
                <FieldGroup label="Middle Name" mb={0}>
                    <IconWrap icon={User}>
                        <input className="reg-input" type="text" value={form.middle_name}
                ...
                <FieldGroup label="Last Name" required mb={0}>
                    <IconWrap icon={User}>
                        <input className="reg-input" type="text" value={form.last_name}
            ...
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap:12, marginBottom:12 }}>
                ...
                <FieldGroup label="Contact Number" required mb={0}>
                    <IconWrap icon={Phone}>
                        <input className="reg-input" type="tel" value={form.contact_number}
                ...
                <FieldGroup label="Date of Birth" required mb={0}>
                    <div style={{ position:"relative" }}>
                        <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", opacity:0.35, pointerEvents:"none", display:"flex", zIndex:1 }}>
                            <CalendarDays size={15} color="#0e2554" strokeWidth={2}/>
                        </span>
            ...
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:12 }}>
                <FieldGroup label="Civil Status" required>
                    <select className="reg-select" value={form.civil_status}
                ...
                <FieldGroup label="Nationality" required>
                    <select className="reg-select" value={form.nationality}
            ...
            {/* Address block */}
            <label className="reg-label">Home Address — Barangay East Tapinac only</label>
            <div style={{ background:"#f8f6f1", border:"1px solid #e4dfd4", borderRadius:7, padding:"14px 16px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap:10, marginBottom:10 }}>
                    <FieldGroup label="House / Unit No." required mb={0}>
                        <IconWrap icon={Hash}>
                            <input className="reg-input" type="text" value={form.house_no}
                                onChange={e => set("house_no", e.target.value)} placeholder="e.g. 12-B"/>
                        </IconWrap>
                    </FieldGroup>
                    <FieldGroup label="Purok" mb={0}>
                        <select className="reg-select" value={form.purok_id}
                    ...
                    <FieldGroup label="Street" required mb={0}>
                        <select className="reg-select" value={form.street_id}
                ...
                {form.street_id === "other" && (
                    <FieldGroup label="Specify Street" required mb={0} style={{ marginBottom:10 }}>
                        <input className="reg-input-bare" type="text" value={form.street_other}
                            onChange={e => set("street_other", e.target.value)}
                            placeholder="Enter your street name"/>
                    </FieldGroup>
                )}

                {/* Row 2 — Barangay / City (locked, shown below for context) */}
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop: form.street_id === "other" ? 0 : 10 }}>
                    <div>
                        <label className="reg-label">Barangay</label>
                        <IconWrap icon={MapPin}><input className="reg-input-locked" value="East Tapinac" readOnly/></IconWrap>
                    </div>
                    <div>
                        <label className="reg-label">City</label>
                        <IconWrap icon={MapPin}><input className="reg-input-locked" value="Olongapo City" readOnly/></IconWrap>
                    </div>
                </div>
            </div>
        </div>
    ...
    // ── STEP 2: ID Verification ───────────────────────────────
    const Step2 = (
        <div className="reg-fadein" style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1px 1fr", gap:0, alignItems:"stretch" }}>

            {/* LEFT — upload inputs */}
            <div style={{ display:"flex", flexDirection:"column", paddingRight:18 }}>
                <FieldGroup label="ID Type" required>
                    <select className="reg-select" value={idType}
                ...
                <FieldGroup label="Upload ID Photo" required mb={0} style={{ flex:1, display:"flex", flexDirection:"column" }}>
                    <input ref={fileInputRef} type="file" accept="image/*"
                        style={{ display:"none" }} onChange={handleFilePick}/>
                    {!idFile ? (
                        <div className="reg-upload-zone" onClick={() => fileInputRef.current?.click()}
                            style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
                            <div style={{ width:44, height:44, borderRadius:"50%", background:"#e8eef8", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:10 }}>
                                <Upload size={20} color="#1a4a8a" strokeWidth={2}/>
                            </div>
                            <div style={{ fontSize:13, fontWeight:600, color:"#1a1a2e", marginBottom:3 }}>
                                Tap to upload
                            </div>
                            <div style={{ fontSize:11.5, color:"#9090aa" }}>JPG or PNG · Max 5 MB</div>
                        </div>
                    ) : (
                        <div className="reg-upload-zone has-file" onClick={() => fileInputRef.current?.click()}
                            style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", padding:"16px 14px", cursor:"pointer" }}>
                            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                                <img src={idPreview} alt="ID preview"
                                    style={{ width:72, height:50, objectFit:"cover", borderRadius:4, border:"1px solid #a8d8bc", flexShrink:0 }}/>
                                <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ fontSize:12.5, fontWeight:600, color:"#1a7a4a", marginBottom:2 }}>✓ Uploaded</div>
                                    <div style={{ fontSize:11, color:"#4a4a6a", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                                        {idFile.name}
                                    </div>
                                    <span style={{ fontSize:11, color:"#1a4a8a" }}>Tap to change photo</span>
                                </div>
                                <button type="button" onClick={e => { e.stopPropagation(); removeFile(); }}
                                    style={{ background:"none", border:"none", cursor:"pointer", color:"#b02020", padding:4, flexShrink:0 }}>
                                    <X size={16}/>
                                </button>
                            </div>
                        </div>
            ...
            {/* DIVIDER */}
            {!isMobile && (
                <div style={{ width:1, background:"#e4dfd4", margin:"0 0" }}/>
            )}

            {/* RIGHT — notice + guidelines */}
            <div style={{ display:"flex", flexDirection:"column", gap:12, paddingLeft:18 }}>

                {/* Verification notice */}
                <div style={{ background:"linear-gradient(135deg,#fff7e0,#fff3d0)", border:"1.5px solid #e8c84a", borderRadius:8, padding:"14px 16px" }}>
                    <div style={{ fontSize:12.5, fontWeight:700, color:"#7a4800", marginBottom:6 }}>
                        Account Verification Required
                    </div>
                    <div style={{ fontSize:12, color:"#8a5800", lineHeight:1.65, marginBottom:10 }}>
                        Upload a valid ID so barangay staff can confirm your residency. Review takes <strong>1–3 business days</strong>.
                    </div>
                    <div style={{ display:"flex", alignItems:"center", gap:8, background:"rgba(184,104,0,0.1)", borderRadius:5, padding:"8px 10px" }}>
                        <CalendarClock size={15} color="#b86800" strokeWidth={2} style={{ flexShrink:0 }}/>
                        <span style={{ fontSize:12, fontWeight:700, color:"#7a4800" }}>
                            Come back in 1–3 business days to log in after approval.
                        </span>
                    </div>
                </div>

                {/* Photo guidelines */}
                <div style={{ background:"#f8f6f1", border:"1px solid #e4dfd4", borderRadius:6, padding:"12px 14px" }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#9090aa", textTransform:"uppercase", letterSpacing:1.2, marginBottom:9 }}>
                        Photo Guidelines
                    </div>
                    {[
                        ...
                        <div key={i} style={{ display:"flex", gap:8, marginBottom:i < arr.length-1 ? 6 : 0, alignItems:"flex-start" }}>
                            <div style={{ width:5, height:5, borderRadius:"50%", background:"#0e2554", flexShrink:0, marginTop:5 }}/>
                            <span style={{ fontSize:11.5, color:"#4a4a6a", lineHeight:1.5 }}>{tip}</span>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <div className="reg-fadein">
            <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap:12, marginBottom:6 }}>
                <FieldGroup label="Password" required>
                    <div style={{ position:"relative" }}>
                        <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", opacity:0.35, display:"flex", zIndex:1 }}>
                            <Lock size={15} color="#0e2554" strokeWidth={2}/>
                        </span>
                        <input className="reg-input" type={showPw?"text":"password"}
                            value={password} onChange={e => { setPassword(e.target.value); setError(""); }}
                            placeholder="Min. 8 chars" style={{ paddingRight:38 }}/>
                        <button type="button" onClick={() => setShowPw(p => !p)}
                            style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", opacity:0.4, display:"flex", padding:2 }} tabIndex={-1}>
                            {showPw ? <EyeOff size={15} color="#333"/> : <Eye size={15} color="#333"/>}
                        </button>
                    </div>
                </FieldGroup>
                <FieldGroup label="Confirm Password" required>
                    <div style={{ position:"relative" }}>
                        ...
                        <input className="reg-input" type={showCf?"text":"password"}
                            value={confirm} onChange={e => { setConfirm(e.target.value); setError(""); }}
                            placeholder="Re-enter" style={{ paddingRight:38 }}/>
                        <button type="button" onClick={() => setShowCf(p => !p)}
                            style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", opacity:0.4, display:"flex", padding:2 }} tabIndex={-1}>
                            {showCf ? <EyeOff size={15} color="#333"/> : <Eye size={15} color="#333"/>}
                        </button>
                    </div>
                </FieldGroup>
            </div>
            ...
            <div style={{ display:"flex", gap:16, flexWrap:"wrap", marginBottom:20 }}>
                {pwChecks.map(r => (
                    <div key={r.label} style={{ display:"flex", alignItems:"center", gap:5 }}>
                        <div style={{ width:7, height:7, borderRadius:"50%", background:r.ok?"#1a7a4a":"#ddd", transition:"background .2s" }}/>
                        <span style={{ fontSize:11, color:r.ok?"#1a7a4a":"#9090aa", transition:"color .2s" }}>{r.label}</span>
                    </div>
                ))}
            </div>

            {/* Declaration */}
            <div style={{ background:"#f8f6f1", border:"1px solid #e4dfd4", borderRadius:7, padding:"13px 16px", marginBottom:18 }}>
                <label style={{ display:"flex", alignItems:"flex-start", gap:11, cursor:"pointer", userSelect:"none" }}
                    onClick={() => setDeclared(d => !d)}>
                    <div className={`reg-checkbox ${declared?"checked":""}`}>
                        {declared && <Check size={11} color="#fff" strokeWidth={3}/>}
                    </div>
                    <p style={{ fontSize:12.5, color:"#4a4a6a", lineHeight:1.65, margin:0 }}>
                        I certify that I am a <strong>bona fide resident of Barangay East Tapinac, Olongapo City</strong> and that all information I provided is true and accurate. I understand that false information may result in account denial.
                    </p>
                </label>
            </div>

            {/* Summary */}
            <div style={{ background:"#edf1fa", border:"1px solid rgba(14,37,84,.15)", borderRadius:7, padding:"12px 16px" }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#0e2554", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10 }}>
                    Registration Summary
                </div>
                {[
                    { label:"Name",    value: fullName },
                    { label:"Birthdate", value: form.date_of_birth ? new Date(form.date_of_birth + 'T00:00:00').toLocaleDateString('en-US', { month:'long', day:'numeric', year:'numeric' }) : "—" },
                    { label:"Email",   value: form.email },
                    { label:"Address", value: `${form.house_no}, ${selectedPurok?.name ?? ""}, ${resolvedStreet}` },
                    { label:"Civil Status", value: form.civil_status },
                    { label:"Nationality", value: form.nationality },
                    { label:"ID Type", value: idType },
                ].map(({ label, value }) => (
                    <div key={label} style={{ display:"flex", gap:12, marginBottom:5 }}>
                        <div style={{ width:58, fontSize:11, color:"#9090aa", fontWeight:600, flexShrink:0 }}>{label}</div>
                        <div style={{ fontSize:12.5, color:"#1a1a2e", wordBreak:"break-word" }}>{value || "—"}</div>
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="reg-root"
            style={{ alignItems: isMobile ? "flex-start" : "center", padding: isMobile ? "20px 12px 36px" : "32px 16px" }}>
            <div className="reg-card">
                {/* Gold top bar */}
                <div style={{ height:4, background:"linear-gradient(90deg,#c9a227,#f0d060,#c9a227)" }}/>

                {/* Header — prominent, centered */}
                <div style={{ background:"linear-gradient(180deg,#0e2554 0%,#163066 100%)", padding:"22px 32px 20px", textAlign:"center" }}>
                    <div style={{ width:72, height:72, borderRadius:"50%", border:"2.5px solid rgba(201,162,39,0.6)", background:"rgba(201,162,39,0.08)", overflow:"hidden", margin:"0 auto 14px", flexShrink:0 }}>
                        <img src="/logo.png" alt="Barangay Seal" style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                    </div>
                    <div style={{ fontSize:9.5, letterSpacing:"2.5px", textTransform:"uppercase", color:"rgba(201,162,39,0.7)", marginBottom:4 }}>
                        Republic of the Philippines
                    </div>
                    <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, color:"#fff", lineHeight:1.2, marginBottom:3 }}>
                        Barangay East Tapinac
                    </div>
                    <div style={{ fontSize:11, color:"rgba(255,255,255,0.55)", letterSpacing:"1.5px", textTransform:"uppercase" }}>
                        CertiFast · Resident Registration
                    </div>
                </div>

                {/* Body */}
                <div style={{ padding: isMobile ? "20px 18px 24px" : "26px 32px 28px" }}>
                    <StepIndicator step={step}/>

                    {/* Step title */}
                    <div style={{ marginBottom:18 }}>
                        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:17, fontWeight:700, color:"#0e2554", marginBottom:3 }}>
                            {step===1 && "Your Information"}
                            {step===2 && "Verify Your Residency"}
                            {step===3 && "Create Your Password"}
                        </div>
                        <div style={{ fontSize:12.5, color:"#9090aa" }}>
                            {step===1 && "Personal details and home address within East Tapinac."}
                            {step===2 && "Upload a valid ID so barangay staff can confirm your residency."}
                            {step===3 && "Almost done — set a secure password for your account."}
                        </div>
                    </div>
                    ...
                    {/* Error */}
                    {error && (
                        <div style={{ background:"#fdecea", border:"1px solid #f5c6c6", borderRadius:5, padding:"10px 14px", marginTop:14, marginBottom:4, display:"flex", alignItems:"center", gap:8 }}>
                            <AlertCircle size={14} color="#b02020" style={{ flexShrink:0 }}/>
                            <span style={{ fontSize:13, color:"#b02020" }}>{error}</span>
                        </div>
                    )}
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", gap:10, marginTop:22 }}>
                        {step > 1
                            ? <button className="reg-btn-ghost" onClick={handleBack}><ChevronLeft size={14}/> Back</button>
                            : <div />
                        }
                        {step < 3
                            ? <button className="reg-btn-primary" onClick={handleNext}>Continue <ChevronRight size={14}/></button>
                            : <button className="reg-btn-primary" onClick={handleSubmit} disabled={isLoading}>
                                {isLoading
                ...
                <div style={{ background:"#0e2554", borderTop:"1px solid rgba(201,162,39,0.2)", padding:"14px 32px", display:"flex", alignItems:"center", justifyContent:"center", gap:14 }}>
                    <span style={{ fontSize:13.5, color:"rgba(255,255,255,0.7)", fontFamily:"'Source Serif 4',serif" }}>
                        Already have an account?
                    </span>
                    <button type="button" onClick={() => navigate("/resident/login")}
                        style={{ background:"rgba(201,162,39,0.18)", border:"1.5px solid rgba(201,162,39,0.55)", borderRadius:5, padding:"7px 18px", fontSize:13.5, fontWeight:700, color:"#f0d060", cursor:"pointer", fontFamily:"'Source Serif 4',serif", transition:"background .15s" }}>
                        Sign in →
                    </button>
                </div>
            </div>
