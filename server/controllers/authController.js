...
async function residentLoginWithSupabase(req, res) {
    ...
    if (!token) {
        ...
        try {
            ...
            const rejectionCommentSelect = columnResult.rows.length
                ? "rejection_comment"
                : "NULL::text AS rejection_comment";

            const result = await pool.query(
                `SELECT resident_id, full_name, email, password_hash, status, ${rejectionCommentSelect}
                 FROM residents
                 WHERE LOWER(email) = $1
                 LIMIT 1`,
                [email],
            ...
            await createAuditLog({
                ...
                description: `Successful legacy login to CertiFast resident portal`,
                ipAddress: req.ip,
            });
            ...
            const legacyToken = jwt.sign(
                ...
                { expiresIn: process.env.JWT_RESIDENT_EXPIRES || "7d" },
            );
            ...
            console.error("residentLegacyLogin error:", err);
            return res.status(500).json({ message: "Server error" });
        }
    }
