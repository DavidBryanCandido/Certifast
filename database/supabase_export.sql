...
CREATE TABLE requests (
  ...
  extra_fields jsonb,
  signatory_snapshot jsonb DEFAULT '{}'::jsonb NOT NULL,
  notes text,
...
CREATE UNIQUE INDEX admin_accounts_supabase_auth_id_unique
ON admin_accounts (supabase_auth_id)
...
CREATE TABLE barangay_terms (
  term_id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  term_name character varying(160) NOT NULL,
  starts_on date,
  ends_on date,
...
ALTER TABLE barangay_terms ADD PRIMARY KEY (term_id);
...
CREATE TABLE barangay_positions (
  ...
  position_group character varying(30) DEFAULT 'staff' NOT NULL,
  default_honorific character varying(30),
  is_elected boolean DEFAULT false NOT NULL,
  is_signatory_eligible boolean DEFAULT false NOT NULL,
  requires_purok boolean DEFAULT false NOT NULL,
  sort_order smallint DEFAULT 0 NOT NULL,
  is_active boolean DEFAULT true NOT NULL,
  created_at timestamp without time zone DEFAULT now() NOT NULL
);

ALTER TABLE barangay_positions ADD PRIMARY KEY (position_id);

CREATE TABLE barangay_personnel (
  personnel_id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
...
ALTER TABLE barangay_personnel ADD PRIMARY KEY (personnel_id);

CREATE TABLE barangay_personnel_assignments (
...
ALTER TABLE barangay_personnel_assignments ADD PRIMARY KEY (assignment_id);
ALTER TABLE barangay_personnel_assignments ADD FOREIGN KEY (personnel_id) REFERENCES barangay_personnel(personnel_id);
ALTER TABLE barangay_personnel_assignments ADD FOREIGN KEY (position_id) REFERENCES barangay_positions(position_id);
ALTER TABLE barangay_personnel_assignments ADD FOREIGN KEY (term_id) REFERENCES barangay_terms(term_id);
ALTER TABLE barangay_personnel_assignments ADD FOREIGN KEY (purok_id) REFERENCES puroks(purok_id);
